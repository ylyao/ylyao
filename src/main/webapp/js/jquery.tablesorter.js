...
(function($) {
	...
	$.fn.tableSorter = function(o) {
		...
		return this.each(function(){
			...
			function sortOnColumn(oCell,dir,index) {
				...
				//扩展修改排序图片的方向
				var imgprice=$(COLUMN_CELL).find("span").html(); 
				if(imgprice == ""){
					$(COLUMN_CELL).parent().find("th").each(function(i){//清空其他列的图片
						if($(this).find("span").html()!=""){
							$(this).children().find("img").remove();
						}
				    });
					$(COLUMN_CELL).find("span").html("<img src='css/images/up-org.png'/>");
				}else{
					$(COLUMN_CELL).find("img")[0].src=dir % 2 ? 'css/images/down-org.png' : 'css/images/up-org.png';
				}
	...
	$.tableSorter = {
		...
		utils: {
			...
			isHeaderDisabled: function(defaults,o,arg,index) {//排序核心代码
